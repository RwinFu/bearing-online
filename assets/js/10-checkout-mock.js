    ...
    container.innerHTML = `
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
            <div class="lg:col-span-2 space-y-6">
                <div class="checkout-step-card overflow-hidden">
                    ...
                    <div class="overflow-x-auto"><table class="w-full text-sm"><thead class="bg-gray-50"><tr><th class="p-3 text-right">کالا</th><th class="p-3">موجودی</th><th class="p-3">تعداد</th><th class="p-3">جمع</th></tr></thead><tbody>
                        ${getCartProducts().map(item => `<tr class="border-t"><td class="p-3"><b dir="ltr">${item.brand} ${item.code}</b><div class="text-gray-400">${item.sell_mode === 'instant' ? 'خرید آنلاین' : 'نیازمند استعلام'}</div></td><td class="p-3">${getStockBadge(item)}</td><td class="p-3">${item.quantity}</td><td class="p-3 font-bold">${item.sell_mode === 'instant' ? formatToman(moneyTomanFromUSD(item.priceUSD) * item.quantity) + ' تومان' : 'RFQ'}</td></tr>`).join('')}
                    </tbody></table></div>
                </div>
                ${quote.length ? `<div class="bg-orange-50 border border-orange-100 rounded-2xl p-5"><b>RFQ جداگانه</b><p class="text-sm text-orange-800 mt-2">${quote.length} قلم استعلامی از پرداخت فوری جدا شد و به پیش‌فاکتور/تأیید فروش متصل می‌شود. برای این اقلام قیمت حدس زده نمی‌شود.</p><button onclick="createRFQFromQuoteItems()" class="mt-4 btn-accent text-white px-5 py-3 rounded-xl font-bold" type="button">ثبت RFQ اقلام استعلامی</button></div>` : ''}
                <div class="checkout-step-card p-5">
                    ...
                    <button onclick="refreshShippingQuotes()" class="mt-4 btn-primary text-white px-5 py-3 rounded-xl font-bold" type="button">محاسبه ارسال</button>
                </div>
                <div class="checkout-step-card p-5">
            ...
            <aside class="checkout-summary lg:sticky lg:top-24 h-max bg-white rounded-2xl shadow-lg p-6">
                ...
                <button onclick="createOrderAndPay()" class="w-full mt-5 btn-accent text-white py-4 rounded-xl font-extrabold ${instant.length ? '' : 'opacity-50'}" ${instant.length ? '' : 'disabled'} type="button">تأیید و پرداخت آنلاین</button>
                <button onclick="downloadLatestProforma()" class="w-full mt-3 border border-gray-200 py-3 rounded-xl font-bold text-gray-700" type="button">دانلود پیش‌فاکتور</button>
            </aside>
        </div>`;
    if (hadTypedAddress) {
        ...
        <div class="p-3 bg-white rounded-xl border border-gray-100">
            <div class="flex flex-col md:flex-row md:items-center justify-between gap-2">
                ...
                <div class="flex gap-2">
                    ...
                    ${editable ? `<button onclick="deleteComplaint('${c.id}')" class="px-3 py-2 rounded-xl border border-red-200 text-red-600 text-xs font-bold" title="حذف شکایت" type="button"><i class="fas fa-trash"></i></button>` : ''}
                </div>
            </div>
    ...
    div.innerHTML = `
        <div class="${ordersPanel ? 'acct-panel' : 'bg-white rounded-2xl p-5 shadow-sm border border-gray-100'}">
            ...
            <form id="complaint-form" onsubmit="submitComplaint(event)" class="grid md:grid-cols-2 gap-3 ${ordersPanel ? '' : 'mt-4'}">
                ...
                <button class="${ordersPanel ? 'acct-btn-primary' : 'btn-accent text-white px-5 py-3 rounded-xl font-bold'} md:col-span-2" type="button">${ordersPanel ? '<i class="fas fa-paper-plane"></i>ثبت شکایت' : 'ثبت شکایت'}</button>
            </form>
        </div>
