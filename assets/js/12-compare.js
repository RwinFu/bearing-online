...
function renderWishlist() {
    ...
    if (!products.length) {
        container.innerHTML = `<div class="bg-white rounded-2xl shadow-sm p-12 text-center"><i class="fas fa-heart text-6xl text-gray-200 mb-5"></i><h3 class="text-2xl font-bold text-gray-800 mb-2" data-en="No saved products" data-fa="محصولی نشان نشده است">${AppState.language === 'fa' ? 'محصولی نشان نشده است' : 'No saved products'}</h3><p class="text-gray-500 mb-6" data-en="Tap the heart on any product to save it here." data-fa="روی آیکن قلب محصولات کلیک کنید تا اینجا ذخیره شوند.">${AppState.language === 'fa' ? 'روی آیکن قلب محصولات کلیک کنید تا اینجا ذخیره شوند.' : 'Tap the heart on any product to save it here.'}</p><button onclick="showPage('search'); AppState.searchResults=ProductDatabase; renderSearchResults();" class="btn-primary text-white px-6 py-3 rounded-xl font-bold" data-en="Browse products" data-fa="مشاهده محصولات" type="button">${AppState.language === 'fa' ? 'مشاهده محصولات' : 'Browse products'}</button></div>`;
        return;
    }
    container.innerHTML = `<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">${products.map(p => `<div class="bg-white rounded-2xl shadow-sm p-6 card-hover cursor-pointer" onclick="showProductDetail('${p.id}')"><div class="flex items-start justify-between gap-4"><div><span class="px-2 py-1 text-xs rounded bg-blue-100 text-blue-700 font-bold">${p.brand}</span><h3 class="text-xl font-extrabold mt-3">${p.code}</h3><p class="text-sm text-gray-500 mt-1">${formatDimensions(p)}</p><div class="mt-3">${getStockBadge(p)}</div></div><button onclick="event.stopPropagation(); toggleWishlist('${p.id}')" class="text-red-500 text-xl" type="button"><i class="fas fa-heart"></i></button></div><div class="mt-5 flex items-center justify-between"><b>${p.sell_mode === 'instant' ? formatPrice(p.priceUSD) + ' ' + currencyLabel() : quoteLabel()}</b>${p.sell_mode === 'instant' ? `<button onclick="event.stopPropagation(); addToCart('${p.id}')" class="btn-primary text-white px-4 py-2 rounded-lg" type="button"><i class="fas fa-cart-plus"></i></button>` : `<button onclick="event.stopPropagation(); requestQuote('${p.id}')" class="btn-accent text-white px-4 py-2 rounded-lg" type="button">${quoteLabel()}</button>`}</div></div>`).join('')}</div>`;
    applyLanguage();
}
            ...
            <table class="w-full min-w-max">
                    ...
                    <tr class="border-b border-gray-100">
                        ...
                        ${products.map(p => `
                            <th class="p-6 text-center min-w-48">
                                ...
                                <button onclick="toggleCompare('${p.id}'); renderCompare();" class="text-red-500 text-sm mt-2 hover:underline" data-en="Remove" data-fa="حذف" type="button">Remove</button>
                            </th>
                        `).join('')}
                    </tr>
                </thead>
                <tbody>
                    ...
                    <tr class="border-b border-gray-100 bg-blue-50">
                        ...
                        ${products.map(p => `<td class="p-4 text-center font-bold text-lg ${p.sell_mode === 'instant' ? 'text-blue-600' : 'text-orange-600'}">${p.sell_mode === 'instant' ? formatPrice(p.priceUSD) + ' ' + currencyLabel() : quoteLabel()}</td>`).join('')}
                    </tr>
                    <tr>
                        <td class="p-4"></td>
                        ${products.map(p => `
                            <td class="p-4 text-center">
                                <button onclick="addToCart('${p.id}')" class="btn-primary text-white px-6 py-3 rounded-lg font-medium" type="button">
